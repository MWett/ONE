...
public class BCNode {
	...
	protected ArrayList<DTNHost> hostList =  new ArrayList<DTNHost>();
	...
	public  void interact(DTNHost n1, DTNHost n2) {
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				n2.checkForMessages(n2.blockchain);
				/*else {
	...
	public BCMessage generateMessage(DTNHost host) {
		int randomNum ;
		PublicKey pk = host.getPublicKey2();
		System.out.println("Created message for: " + host.getAddress());
	...
	/*		int randomNum = ThreadLocalRandom.current().nextInt(0, hostList.size());
	PublicKey pk = getPublicKeys().get(randomNum);
	DTNHost dtn = hostList.get(randomNum);
	pk = dtn.getPublicKey2();*/
	public ArrayList<PublicKey> exchangeKeys(ArrayList<PublicKey> keys1, ArrayList<PublicKey> keys2) {
		...
		Set<PublicKey> fooSet = new LinkedHashSet<PublicKey>(keys1);
		fooSet.addAll(keys2);
